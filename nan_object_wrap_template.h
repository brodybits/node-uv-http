...
template <typename T>
class ObjectWrapTemplateBase : public Nan::ObjectWrap {
 protected:
  static inline v8::Local<v8::FunctionTemplate>
  NewConstructorFunctionTemplate(const char * object_class_name,
                                 int internal_field_count) {
    v8::Local<v8::FunctionTemplate> tpl = Nan::New<v8::FunctionTemplate>(New);
    tpl->SetClassName(Nan::New(object_class_name).ToLocalChecked());
  ...
  SetNewFunction(Nan::ADDON_REGISTER_FUNCTION_ARGS_TYPE target,
    ...
    Nan::Set(target, Nan::New(object_class_name).ToLocalChecked(), tpl->GetFunction());
  }

  ObjectWrapTemplateBase() {}
  ~ObjectWrapTemplateBase() {}

  // Used by NewConstructorFunctionTemplate():
  // (same as static NAN_METHOD(New) { ... })
  static void New(Nan::NAN_METHOD_ARGS_TYPE info) {
    if (info.IsConstructCall()) { /* should always be the case here */
      T *obj = new T(info);
      obj->Wrap(info.This());
...
template <typename T>
class ObjectWrapTemplate : public ObjectWrapTemplateBase<T> {
 protected:
  static inline void
  ...
  static inline void NewInstanceMethod(v8::Local<v8::Function> cons, Nan::NAN_METHOD_ARGS_TYPE info) {
    ObjectWrapTemplateBase<T>::NewInstanceMethod(cons, info);
  }

  static inline v8::Local<v8::Value> NewInstanceMethod(v8::Local<v8::Function> cons, int argc, v8::Local<v8::Value> argv[]) {
    ObjectWrapTemplateBase<T>::NewInstanceMethod(cons, argc, argv);
  }
